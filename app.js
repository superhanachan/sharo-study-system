...
class QuizApp {
    ...
    activateFirstVisibleBank() {
        ...
        const centerY = window.innerHeight * 0.4; // Target slightly above center for more natural focus

        targets.forEach(tr => {
            const rect = tr.getBoundingClientRect();
            // Check if any part of the row is in the "focus area" (middle 60% of screen)
            const isInFocusArea = rect.top < window.innerHeight * 0.8 && rect.bottom > window.innerHeight * 0.2;

            if (isInFocusArea) {
                // Calculate distance from the row's "reading start" to our target focus line
                // For short rows, center is fine. For long rows, top is better.
                const rowFocusPoint = rect.height < window.innerHeight ? (rect.top + rect.height / 2) : rect.top + 100;
        ...
        // Fallback: if no row in focus area, find the closest one to the screen
        if (!bestRow) {
            targets.forEach(tr => {
                const rect = tr.getBoundingClientRect();
        ...
        if (!bestRow) bestRow = targets[0];

        if (bestRow && this._currentActiveRowId !== bestRow.id) {
