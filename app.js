...
class QuizApp {
    constructor() {
        ...
        this.currentSetId = (this.quizData && this.quizData.length > 0) ? this.quizData[0].id : null;
    ...
    loadData() { 
        const saved = localStorage.getItem('sharoQuizData');
        const data = saved ? JSON.parse(saved) : null;
        return (data && data.length > 0) ? data : JSON.parse(JSON.stringify(DEFAULT_QUIZ_DATA));
    }
    saveData() {
        if (!this.quizData || this.quizData.length === 0) {
    ...
    checkAndOfferRecovery(manual = false) {
        // ユーザーの要望により、起動時の自動復旧ポップアップを無効化（誤操作・初期化防止）
        if (!manual) return;

        const backupStr = localStorage.getItem('sharo_auto_backup');
        if (!backupStr) {
            alert('内部バックアップが見つかりません。');
            return;
        }

        try {
            const backup = JSON.parse(backupStr);
            const date = new Date(backup.timestamp || backup.savedAt).toLocaleString();
            const msg = `内部バックアップ（保存日時: ${date}）からデータを復元しますか？\n現在編集中の内容は上書きされます。`;
