...
class QuizApp {
    ...
    normalizeInput(str) {
        if (!str) return "";
        return str.trim().toLowerCase().replace(/[Ａ-Ｚａ-ｚ０-９]/g, (s) => String.fromCharCode(s.charCodeAt(0) - 0xFEE0));
    }

    shouldAutoFill(statKey) {
        if (!this.autoFillEnabled) return false;
        // If threshold is 0, consider it learned immediately
        ...
        const lastN = stat.recent.slice(-this.autoFillThreshold);
        return lastN.length === this.autoFillThreshold && lastN.every(r => r === 1);
    }
