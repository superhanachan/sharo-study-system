...
class QuizApp {
    ...
    showSidebarDeleteConfirmModal() {
        if (!this.isEditMode) return;
        const targets = this.quizData.filter(item => item.isInPool);
        
        if (targets.length === 0) {
            alert('削除する項目が選択されていません。（サイドバーのチェックボックスをオンにしてください）');
            return;
        }
    ...
    executeDeleteSelectedQuestions() {
        ...
        // Also clean up statistics for deleted items to avoid clutter
        idsToDelete.forEach(id => {
            const baseId = this.getQuestionBaseId(id);
            // Simple cleanup: delete summary key
            delete this.questionStats[this.getSummaryStatKey(baseId, 'page')];
            // Also could delete blanks but they are harder to track by ID prefix alone without iterating all keys
        });
